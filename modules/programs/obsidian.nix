...
in 
{
  ...
  config = mkIf cfg.enable {
    ...
    home.file = let
      vaultFiles = mapAttrsToList (vaultName: vaultCfg: let
        enabled = vaultCfg.enable;
        enabledCorePlugins = filterAttrs (_: p: p.enable) vaultCfg.core-plugins;
        corePluginsList = attrNames enabledCorePlugins;
        enabledCommunityPlugins = filterAttrs (_: p: p.enable) vaultCfg.community-plugins;
        ...
        coreFile = if corePluginsList != [] then { "${obsidianDir}/core-plugins.json".text = corePluginsJson; } else {};
        communityFile = if communityPluginsList != [] then { "${obsidianDir}/community-plugins.json".text = communityPluginsJson; } else {};
        appFile = if appJson != {} then { "${obsidianDir}/app.json".text = builtins.toJSON appJson; } else {};
        
        communityDataFiles = foldl' (acc: pn: let
          config = enabledCommunityPlugins.${pn}.config;
        in
          if config != {} then
            acc // { "${obsidianDir}/plugins/${pn}/data.json".text = builtins.toJSON config; }
          else
            acc
        ) {} communityPluginsList;
        
        extraConfigFiltered = removeAttrs vaultCfg.extraConfig [ "app" ];
        extraConfigFiles = mapAttrs' (fn: content: nameValuePair
          "${obsidianDir}/${fn}.json" { text = builtins.toJSON content; }
        ) (filterAttrs (_: content: content != {}) extraConfigFiltered);
        
        allFiles = coreFile // communityFile // appFile // communityDataFiles // extraConfigFiles;
      in
        mkIf enabled allFiles
      ) cfg.vaults;
    in
      foldl' (a: b: a // b) {} vaultFiles;
  };
}